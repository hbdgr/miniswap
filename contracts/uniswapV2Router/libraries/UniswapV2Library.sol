...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address) {
        ...
        // init code (hash of UniswapV2Pair bytecode)
        // dynamically discovered, instead of hardcoded hex"string"
        return address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            IPairCode(address(this)).pairCodeHash() // function from contract which uses this library
                        )
                    )
                )
            )
